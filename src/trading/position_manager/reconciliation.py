...
def track_recent_fill(
    ...
    if timestamp is None:
        ...
        # Handle ISO format timestamp strings (e.g., "2026-01-16T09:02:15.86502Z")
        try:
    ...
    log(
        f"   🔍 RECONCILIATION AUDIT: Tracking recent fill for order {order_id[:10]} | size={size}, price=${price}, timestamp={timestamp}"
    )
...
def is_recently_filled(order_id: str, max_age_seconds: int = 300) -> bool:
    ...
    if is_recent:
        log(
            f"   🔍 RECONCILIATION AUDIT: Order {order_id[:10]} recently filled {age_seconds}s ago (within {max_age_seconds}s threshold)"
        )
...
def safe_cancel_order(order_id: str, context: str = "") -> bool:
    ...
    log(
        f"   🔍 RECONCILIATION AUDIT: [{context}] Starting safe cancellation for order {order_id[:10]}"
    )
    ...
    try:
        ...
        if is_recently_filled(order_id):
            ...
            log(
                f"   ✅ [{context}] RACE CONDITION PREVENTED: Order {order_id[:10]} was recently filled (size={fill_size}, price=${fill_price}), skipping cancellation"
            )
            return True
        ...
        log(
            f"   🔍 RECONCILIATION AUDIT: [{context}] Order {order_id[:10]} current status: {status}"
        )
        ...
        if status in ["FILLED", "MATCHED", "CANCELED", "EXPIRED", "NOT_FOUND"]:
            if status in ["FILLED", "MATCHED"]:
                ...
                log(
                    f"   ✅ [{context}] Order {order_id[:10]} already filled (Status: {status}), skipping cancellation"
                )
            elif status in ["CANCELED", "EXPIRED"]:
                log(
                    f"   🧹 [{context}] Order {order_id[:10]} already cancelled/expired (Status: {status}), no action needed"
                )
            return True
        ...
        if order_data:
            ...
            if float(size) > 0:
                ...
                log(
                    f"   ✅ [{context}] Order {order_id[:10]} has matched size {size}, skipping cancellation"
                )
                return True
        ...
        log(
            f"   🔧 [{context}] Attempting cancellation for order {order_id[:10]} (Status: {status})"
        )
        ...
        if result:
            ...
            log(
                f"   ✅ [{context}] Successfully cancelled order {order_id[:10]} (Status: {status})"
            )
        else:
            # AUDIT: Cancellation failed
            log(
                f"   ❌ [{context}] Failed to cancel order {order_id[:10]} (Status: {status})"
            )
...
def verify_order_unfilled(order_id: str, max_retries: int = 2) -> bool:
    ...
    log(
        f"   🔍 RECONCILIATION AUDIT: Starting verification for order {order_id[:10]} (max_retries={max_retries})"
    )
    ...
    if is_recently_filled(order_id):
        ...
        log(
            f"   ⚠️  RECONCILIATION AUDIT: Order {order_id[:10]} found in recent fills cache, treating as filled"
        )
        return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            log(
                f"   🔍 RECONCILIATION AUDIT: Verification attempt {attempt + 1}/{max_retries} for order {order_id[:10]}"
            )
            ...
            log(
                f"   🔍 RECONCILIATION AUDIT: Order {order_id[:10]} status on attempt {attempt + 1}: {status}"
            )
            ...
            if status in ["FILLED", "MATCHED", "CANCELED", "EXPIRED", "NOT_FOUND"]:
                ...
                log(
                    f"   ✅ RECONCILIATION AUDIT: Order {order_id[:10]} confirmed as completed (Status: {status})"
                )
            ...
            if status in ["LIVE", "PENDING", "OPEN"]:
                ...
                log(
                    f"   ✅ RECONCILIATION AUDIT: Order {order_id[:10]} confirmed as open (Status: {status})"
                )
                return True
