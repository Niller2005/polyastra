...
def track_recent_fill(order_id: str, price: float = None, size: float = None, timestamp: int = None):
    ...
    log(f"   🔍 RECONCILIATION AUDIT: Tracking recent fill for order {order_id[:10]} | size={size}, price=${price}, timestamp={timestamp}")
...
def is_recently_filled(order_id: str, max_age_seconds: int = 300) -> bool:
    ...
    fill_data = _recently_filled_orders.get(order_id)
    if not fill_data:
        return False
    ...
    if is_recent:
        log(f"   🔍 RECONCILIATION AUDIT: Order {order_id[:10]} recently filled {age_seconds}s ago (within {max_age_seconds}s threshold)")
...
def get_recent_fill_data(order_id: str) -> Optional[Dict[str, Any]]:
    ...
    _cleanup_recent_fills()
    return _recently_filled_orders.get(order_id)
...
def _cleanup_recent_fills():
    ...
    # Remove old entries
    old_orders = [order_id for order_id, data in _recently_filled_orders.items() 
                  if data['timestamp'] < cutoff_time]
    
    for order_id in old_orders:
        # AUDIT: Cleaning up old fill tracking
        del _recently_filled_orders[order_id]
        log(f"   🧹 RECONCILIATION AUDIT: Cleaned up old fill tracking for order {order_id[:10]}")
...
def safe_cancel_order(order_id: str, context: str = "") -> bool:
    ...
    log(f"   🔍 RECONCILIATION AUDIT: [{context}] Starting safe cancellation for order {order_id[:10]}")
    ...
    try:
        ...
        if is_recently_filled(order_id):
            ...
            log(f"   ✅ [{context}] RACE CONDITION PREVENTED: Order {order_id[:10]} was recently filled (size={fill_size}, price=${fill_price}), skipping cancellation")
            return True
        ...
        log(f"   🔍 RECONCILIATION AUDIT: [{context}] Order {order_id[:10]} current status: {status}")
        ...
        if status in ["FILLED", "MATCHED", "CANCELED", "EXPIRED", "NOT_FOUND"]:
            if status in ["FILLED", "MATCHED"]:
                ...
                log(f"   ✅ [{context}] Order {order_id[:10]} already filled (Status: {status}), skipping cancellation")
            elif status in ["CANCELED", "EXPIRED"]:
                log(f"   🧹 [{context}] Order {order_id[:10]} already cancelled/expired (Status: {status}), no action needed")
            return True
        ...
        if order_data:
            ...
            if float(size) > 0:
                ...
                log(f"   ✅ [{context}] Order {order_id[:10]} has matched size {size}, skipping cancellation")
                return True
        ...
        log(f"   🔧 [{context}] Attempting cancellation for order {order_id[:10]} (Status: {status})")
        
        result = cancel_order(order_id)
        
        if result:
            # AUDIT: Cancellation successful
            log(f"   ✅ [{context}] Successfully cancelled order {order_id[:10]} (Status: {status})")
        else:
            # AUDIT: Cancellation failed
            log(f"   ❌ [{context}] Failed to cancel order {order_id[:10]} (Status: {status})")
        
        return result
        
    except Exception as e:
        # AUDIT: Error during cancellation process
        log(f"   ❌ [{context}] ERROR in safe_cancel_order for {order_id[:10]}: {e}")
        return False
...
def verify_order_unfilled(order_id: str, max_retries: int = 2) -> bool:
    ...
    log(f"   🔍 RECONCILIATION AUDIT: Starting verification for order {order_id[:10]} (max_retries={max_retries})")
    
    # First check recent fills cache
    if is_recently_filled(order_id):
        # AUDIT: Order found in recent fills cache
        log(f"   ⚠️  RECONCILIATION AUDIT: Order {order_id[:10]} found in recent fills cache, treating as filled")
        return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            log(f"   🔍 RECONCILIATION AUDIT: Verification attempt {attempt + 1}/{max_retries} for order {order_id[:10]}")
            
            status = get_order_status(order_id)
            
            # AUDIT: Status check result
            log(f"   🔍 RECONCILIATION AUDIT: Order {order_id[:10]} status on attempt {attempt + 1}: {status}")
            
            # Order is definitely completed
            if status in ["FILLED", "MATCHED", "CANCELED", "EXPIRED", "NOT_FOUND"]:
                # AUDIT: Order confirmed as completed
                log(f"   ✅ RECONCILIATION AUDIT: Order {order_id[:10]} confirmed as completed (Status: {status})")
            ...
            if status in ["LIVE", "PENDING", "OPEN"]:
                ...
                log(f"   ✅ RECONCILIATION AUDIT: Order {order_id[:10]} confirmed as open (Status: {status})")
                return True
            
            # Unknown status, try again
            if attempt < max_retries - 1:
                # AUDIT: Retrying verification
                log(f"   ⏳ RECONCILIATION AUDIT: Unknown status for {order_id[:10]}, retrying in 0.1s...")
                time.sleep(0.1)  # Brief delay before retry
                
        except Exception as e:
            if attempt < max_retries - 1:
                # AUDIT: Error on attempt, retrying
                log(f"   ⚠️  RECONCILIATION AUDIT: Error on attempt {attempt + 1} for {order_id[:10]}: {e}, retrying...")
                time.sleep(0.1)  # Brief delay before retry
            else:
                # AUDIT: Final verification failure
                log(f"   ❌ RECONCILIATION AUDIT: Failed to verify order status for {order_id[:10]} after {max_retries} attempts: {e}")
                return False
    ...
    log(f"   ⚠️  RECONCILIATION AUDIT: Inconclusive verification for {order_id[:10]} after {max_retries} attempts, assuming unfilled for safety")
    return True
