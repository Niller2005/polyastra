...
from src.config.settings import ENABLE_EXIT_PLAN, EXIT_PRICE_TARGET, EXIT_MIN_POSITION_AGE
...
def _update_exit_plan_after_scale_in(
    ...
    if cancel_order(old_order_id):
        ...
        if res["success"]:
            ...
            log(f"   ✅ [{symbol}] #{trade_id} Updated exit plan for new size: {truncated_size:.2f} (ID: {oid_str}...)")
            return True
        else:
...
def _update_exit_plan_to_new_price(
    ...
    if cancel_order(old_order_id):
        ...
        if res["success"]:
            ...
            log(f"   ❌ [{symbol}] #{trade_id} Failed to update exit plan price: {res.get('error')}")
            return False
    return False
        ...
        if o_data:
            ...
            if o_status in ["FILLED", "MATCHED"]:
                ...
                if truncate_float(o_size, 2) != truncate_float(size, 2):
                    log(f"   🔧 [{symbol}] #{trade_id} Exit plan size mismatch: {o_size} != {size}. Repairing...")
                    _update_exit_plan_after_scale_in(symbol, trade_id, token_id, size, limit_sell_id, c, conn)
            elif o_status in ["CANCELED", "EXPIRED"]:
                log(f"   ⚠️ [{symbol}] #{trade_id} Exit plan order was {o_status}. Clearing from DB to allow retry.")
                c.execute("UPDATE trades SET limit_sell_order_id = NULL WHERE id = ?", (trade_id,))
                limit_sell_id = None
        else:
            # Order not found on exchange
            log(f"   ⚠️ [{symbol}] #{trade_id} Exit plan order {limit_sell_id[:10]}... not found on exchange. Clearing.")
            c.execute("UPDATE trades SET limit_sell_order_id = NULL WHERE id = ?", (trade_id,))
            limit_sell_id = None
