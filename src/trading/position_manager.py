...
def handle_expired_positions():
    """
    Handle positions that have expired but not yet settled.
    Place limit sell orders at current market price for tokens we still hold.
    """
    conn = sqlite3.connect(DB_FILE, timeout=30.0)
    c = conn.cursor()
    now = datetime.now(tz=ZoneInfo("UTC"))

    # Get unsettled trades where window has ENDED but trade is not settled or exited
    c.execute(
        """SELECT id, symbol, token_id, size, bet_usd, limit_sell_order_id, order_status
           FROM trades 
           WHERE settled = 0 
           AND exited_early = 0
           AND datetime(window_end) <= datetime(?)""",
    ...
    log(f"🔍 Handling {len(expired_positions)} expired positions...")
    ...
    for (
        ...
        limit_sell_order_id,
        order_status,
    ) in expired_positions:
        try:
            # Only handle FILLED orders that don't have a limit sell order
            if order_status == "FILLED" and not limit_sell_order_id:
                log(
                    f"[{symbol}] ⏰ Window expired for trade #{trade_id}, placing limit sell at market price"
                )
                ...
                if not bids:
                    log(
                        f"[{symbol}] ⚠️ No bids available for trade #{trade_id}, will retry next cycle"
                    )
                    continue

                # Get best bid price (what we can sell for immediately)
                best_bid = float(
                    bids[-1].price
                ...
                # Place limit sell at current bid price (or higher if very profitable)
                sell_price = max(best_bid, 0.50)  # Don't sell for less than $0.50

                log(
                    f"[{symbol}] 📉 Placing limit sell order at ${sell_price:.4f} for {size} units"
                )
                sell_limit_result = place_limit_order(
                    token_id, sell_price, size, SELL, silent_on_balance_error=True
                )

                if sell_limit_result["success"]:
                    limit_sell_order_id = sell_limit_result["order_id"]
                    log(
                        f"[{symbol}] ✅ Limit sell order placed at ${sell_price:.4f}: {limit_sell_order_id}"
                    )
                    c.execute(
                    ...
                    log(
                        f"[{symbol}] ⚠️ Failed to place limit sell for trade #{trade_id} (will retry next cycle)"
                    )

        except Exception as e:
            log(f"⚠️ Error handling expired position #{trade_id}: {e}")
